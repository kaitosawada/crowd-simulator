...
- 初期500人、0〜1000人のNPCが構内に最初からいて、二階を時計回り・反時計回りに一周して、階段を下り改札へ帰ります。帰ったNPCは改札で消え、反対側の改札から再び出現します。体格・服装・速度に個体差があり、移動距離に合わせて手足が動きます。
- 人との接近を予測する回避と、回避を行わない経路追従を切り替えられます。
- 人数、速度、停止／再開、リセット、進行方向の色分け、代表経路の表示を操作できます。
- 俯瞰モードでは天井が非表示になり、全体の流れを観察できます。
- 現在階に応じたミニマップ、構内の人数と帰駅者数、平均歩行速度、シミュレーション内の経過時間、描画FPSを表示します。
