...
- 初期1000人、0〜1000人のNPCが両方の階に最初からいます。通り抜け（約40%）、買い物・飲食（約50%）、散策（約10%）を混ぜ、出入口と目的地を個別に選びます。買い物客は1〜2店舗に寄り、店先で滞在してから移動を再開します。滞在はキオスク約8〜18秒、カフェ約32〜68秒、書店約21〜45秒、マーケット約15〜33秒です。退出後は別の入口から新しい用事で出現します。体格・服装・速度にも個体差があり、移動距離に合わせて手足が動きます。
...
- 現在階に応じたミニマップ、構内の人数と滞在人数・退出者数、平均歩行速度、シミュレーション内の経過時間、描画FPSを表示します。
...
src/
  ...
  simulation/                Three.js / DOMに依存しないシミュレーション
    ...
    journey.ts               出入口・階段・店舗を結ぶ有限経路と滞在地点
...
二階の店舗はファサードで、店内の移動はなく、買い物・飲食は店先での滞在として表現します。ホームへの移動、目的地ごとの汎用経路探索、アニメーション付きの外部人体モデルも対象外です。3Dモデルとテクスチャはローカルで生成し、フォントはシステムフォントを使います。外部の素材やフォントのダウンロードは不要です。

## 検証

`npm test` は全出入口の組み合わせと店先への経路の通行可能性、48人の600秒間の滞在・再出発・全6出口への退出、全8店舗の利用、階段の連続した昇降と転落防止を検証します。既存の壁・家具の衝突、空間ハッシュ、倍速／リセット、プレイヤーへの予測回避、独立したカスタム行動、500人の状態の健全性も確認します。本番ビルドではTypeScriptの型チェックも実施します。
