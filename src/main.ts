...
import { createLaneGuide } from './render/LaneGuide';
...
function rebuildLanes() {
  ...
  laneMesh = createLaneGuide(simulation.route, simulation.laneSeparation);
  $('lane-stair-legend').textContent = simulation.laneSeparation ? '緑 ↑ 上り · オレンジ ↓ 下り' : '紫 ↕ 上下共用';
  laneGroup.add(laneMesh);
}
$('lane-guide-toggle').addEventListener('change', e => {
  const visible = (e.target as HTMLInputElement).checked;
  laneGroup.visible = visible;
  $('lane-legend').hidden = !visible;
