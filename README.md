...
- 外寸100 × 68m、中央施設68 × 36m、幅16mの二階建コンコースです。東西の階段でプレイヤーとNPCが昇降できます（二階の床高4.2m）。
- 一階に東西の改札、二階にキオスク・カフェ・書店・マーケットの店舗ファサードがあります。
- 木製ルーバー天井、吊り下げ案内板、ガラス壁、ステンレス柱、点字ブロック、店舗ファサード、植栽、ベンチを配置しています。
- 初期48人、0〜500人のNPCが一階の改札から順次出発し、二階を時計回り・反時計回りに一周して、階段を下り同じ改札へ帰ります。戻ったNPCは出発待ちの最後尾へ入り、再び出発します。設定人数は出発待ちを含みます。体格・服装・速度に個体差があり、移動距離に合わせて手足が動きます。
- 人との接近を予測する回避と、回避を行わない経路追従を切り替えられます。
- 人数、速度、停止／再開、リセット、進行方向の色分け、代表経路の表示を操作できます。
- 俯瞰モードでは天井が非表示になり、全体の流れを観察できます。
- 現在階に応じたミニマップ、構内と改札待ちの人数、平均歩行速度、シミュレーション内の経過時間、描画FPSを表示します。
...
二階の店舗はファサードで、店内への入店や買い物行動は含めていません。ホームへの移動、目的地ごとの汎用経路探索、アニメーション付きの外部人体モデルも対象外です。3Dモデルとテクスチャはローカルで生成し、フォントはシステムフォントを使います。外部の素材やフォントのダウンロードは不要です。

## 検証

`npm test` は東西の階段と両方向の改札往復経路、48人全員の二階周回と帰還、階の分離、プレイヤーの昇降、転落防止に加え、閉経路と全レーンの連続性・通行可能性、静的障害物の制約、空間ハッシュの境界、24人の一周以上の歩行、停止／倍速／リセット、プレイヤーへの予測回避、独立したカスタム行動、500人の状態の健全性を検証します。本番ビルドではTypeScriptの型チェックも実施します。
