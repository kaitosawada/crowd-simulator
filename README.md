...
- 外寸100 × 68m、中央施設68 × 36m、幅16mの二階建コンコースです。東西の階段でプレイヤーとNPCが昇降できます（二階の床高6m、屋根高12.6m）。
- 一階に東西の改札と北口・南口各2か所の計6つの出入口、二階の南北にキオスク・カフェ・書店・マーケットの計8店舗があります。
- 幅8mの採光帯を備えた高い木製ルーバー天井、吊り下げ案内板、ガラス壁、ステンレス柱、点字ブロック、店舗ファサード、植栽、ベンチを配置しています。
- 初期1000人、0〜1000人のNPCが両方の階に最初からいます。通り抜け（約40%）、買い物・飲食（約50%）、散策（約10%）を混ぜ、出入口と目的地を個別に選びます。買い物客は1〜2店舗に寄り、店内のレジに並び、商品棚・カフェのテーブルへ移動して滞在し、別の出口から退店します。滞在はキオスク約8〜18秒、カフェ約32〜68秒、書店約21〜45秒、マーケット約15〜33秒です。退出後は別の入口から新しい用事で出現します。体格・服装・速度にも個体差があり、移動距離に合わせて手足が動きます。
- 人との接近を予測する回避と、回避を行わない経路追従を切り替えられます。
- 南北の広い通路では個体ごとの経路を約6mの幅に分散させ、角から階段脇にかけて徐々に幅を絞ります。
- 階段脇へ押し出されたNPCは入口へ回り込み、実際の登降を確認して経路を進めます。平地の移動中に階段へ押し戻された場合は、元の階へ戻って移動を再開します。
- 人数、速度、リセット、目的別の色分け、代表経路の表示を操作できます。
- 「レーンを帯で表示」で通路のレーン範囲を青い帯、階段の上りを緑・下りをオレンジの矢印付きの帯で表示します。階段の帯は各段に沿い、レーン分離をオフにすると紫の上下共用帯になります。
- 俯瞰モードでは天井が非表示になり、全体の流れを観察できます。
- 現在階に応じたミニマップ、構内・店内・レジ待ちの人数と退出者数、平均歩行速度、シミュレーション内の経過時間、描画FPSを表示します。
...
設定を開くとプレイヤーの移動と見回しを止めます。閉じるとゲーム画面にフォーカスが戻り、移動を再開できます。設定の入力中は移動ショートカットを抑止します。歩行者はプレイヤーも回避対象として扱います。階段はWASDでそのまま昇降でき、側面や二階の開口部からは落ちません。異なる高さにいる人は回避対象から外します。壁・柱・ベンチ・植栽・改札機にはプレイヤーとNPCの両方の当たり判定があります。
...
src/
  ...
  simulation/                Three.js / DOMに依存しないシミュレーション
    ...
    layout.ts                駅寸法、階段、階の遷移、共通の障害物と当たり判定
    journey.ts               出入口・階段・店舗を結ぶ有限経路
    ShopVisits.ts            店内の先着順行列、レジ、滞在場所の予約と退店
...
駅の反復部材は材質ごとに結合しています。NPCは部位ごとのInstancedMeshで描画し、人数に応じてドローコールが増えない構成です。近傍探索は空間ハッシュで絞り込みます。1000人まで設定できますが、FPSはGPU、解像度、ブラウザ、アルゴリズムに依存します。
...
二階の各店舗は13.5 × 8mで、入口と出口、レジ、6人分の行列位置、6か所の滞在場所があります。レジ処理は2.5秒で、滞在場所は退店完了まで予約します。レジ列の定員には入店途中の人も含め、満員の場合はその店への立ち寄りを見送ります。行列は専用の通路を先着順に進みます。飲食はテーブル前での立ち止まりとして表現し、着席アニメーションや商品の受け渡しはありません。ホームへの移動、目的地ごとの汎用経路探索、アニメーション付きの外部人体モデルも対象外です。3Dモデルとテクスチャはローカルで生成し、フォントはシステムフォントを使います。外部の素材やフォントのダウンロードは不要です。

## 検証

`npm test` は店内の入退店と家具の衝突、先着順のレジ処理、満員時の見送り、人数変更・リセット・レーン切替時の予約管理、全出入口の組み合わせと店舗への経路の通行可能性、48人の600秒間の滞在・再出発・全6出口への退出、全8店舗の利用、階段の連続した昇降と転落防止を検証します。既存の壁・家具の衝突、空間ハッシュ、倍速／リセット、プレイヤーへの予測回避、独立したカスタム行動、500人の状態の健全性も確認します。本番ビルドではTypeScriptの型チェックも実施します。

ブラウザでは歩行・俯瞰・人数変更（1000人）・速度・アルゴリズム切り替え・色分け・経路表示を確認しています。
